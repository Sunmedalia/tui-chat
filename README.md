...
双方会看到一组安全码。通过电话、当面或其他可信渠道比较，确认完全一致后，双方分别输入：
...
彻底删除账号需要一份已存在的备份、`--yes` 以及交互输入用户名：

```console
docker compose exec server tui-chat-server db backup /data/before-purge.db
docker compose exec -it server tui-chat-server user purge alice \
  --backup /data/before-purge.db --yes
```
