...
`tui-chat` 是一个使用 Rust 编写的终端一对一聊天工具。客户端主动连接公网中继，因此家庭电脑、NAS 和 NAT 后的服务器都不需要开放入站端口。

消息使用 Olm 双棘轮按设备端到端加密。服务端负责账号管理、在线连接、离线密文保存和消息转发，但无法读取消息正文。

> 当前项目是可运行的安全 MVP，尚未经过独立安全审计。不要将它视为 Signal 等成熟产品的等价替代品。

> 当前网络协议为 **v2**，新建连接请使用 `/v2/ws`。当前客户端会兼容地将命令行中的旧 `/v1/ws` 改写为 `/v2/ws`，但 v1 客户端本身无法连接 v2 服务端。
...
- 离线消息、永久密文历史和自动补拉。
- 发送中、已发送、已送达和已读状态。
- 账号由服务端管理员创建，不开放客户端注册接口。
- 每台设备使用独立身份密钥和 Olm 预密钥。
- 新设备必须由已有设备通过 SAS 短认证码批准。
- 本地消息、草稿、Olm 状态和私钥使用随机 VaultKey 加密。
- 本地口令通过 Argon2id 解锁 VaultKey，不直接用于逐条加密消息。
- WSS、系统 CA 校验和可选 SPKI 公钥固定，降低中间人攻击风险。
- 持久化 outbox、同步游标、心跳和指数退避重连。
- Unicode 编辑、命令补全、鼠标滚动、加密草稿和响应式单栏布局。
...
- Rust 1.95；仓库包含固定工具链和 vendored `protoc`。
- Docker 与 Docker Compose，用于公网部署。
- 推荐使用支持 UTF-8、真彩色和鼠标事件的现代终端。
...
需要同时打开三个终端：一个运行服务端，两个分别运行 Alice 和 Bob。请保持服务端终端持续运行。
...
不要让两个账号共用同一个 `--data-dir`。本地档案会绑定创建它的服务器地址和账号。

`--data-dir` 后面只能跟一个完整目录参数。例如应写成 `--data-dir ./data/alice-client`，不要写成 `--data-dir ./data/ alice-client`。
...
- 本地口令只解锁当前电脑上的加密数据库。
- 账号密码由服务器验证，主要用于首次添加设备、待批准设备和密码重置。
- 忘记本地口令后，服务端无法恢复本机消息或私钥。
...
Bob 会在通知 Tab 看到 Alice 的会话请求，按 `Enter` 接受。双方会看到一组安全码。通过电话、当面或其他可信渠道比较，确认完全一致后，双方分别输入：

```text
/verify
```

联系人前面从 `?` 变成 `✓` 后即可发送消息。不要在未经比较的情况下直接核验，否则无法排除服务端替换身份密钥的攻击。
...
侧栏顶部有“会话”和“通知”两个 Tab，可直接点击切换。通知 Tab 会集中显示多设备配对、会话请求和处理结果，并始终按创建时间从新到旧排列，选中或标记已读不会改变顺序。通知详情提供可点击的“接受”“拒绝”“已核对，认证”“开始配对”和“确认 SAS”操作，也保留 `Enter`、`r`、`d` 键盘操作。联系人认证前必须通过电话、当面等可信渠道比较安全码；设备配对也必须比较 SAS 短认证码，点击操作不会跳过这些安全确认。
...
- 点击侧栏顶部的“会话”或“通知”切换 Tab；通知详情中的“接受”“拒绝”“已核对，认证”“开始配对”“确认 SAS”按钮可直接执行对应操作。
...
删除会话只会清理当前设备上的聊天记录、草稿、联系人入口、相关会话通知和对应待发消息。它不会撤回已经发送的消息，也不会删除对方设备上的记录或服务端仍在保留期内的端到端密文。以后再次收到对方消息时，客户端会加密保存但隐藏正文，并在通知 Tab 生成与 `/chat` 相同的认证请求；接受请求、通过可信渠道比较安全码并认证后，消息才会显示。连续收到多条待认证消息只生成一条会话通知。
...
输入 `/` 后可以使用 `Tab` 补全命令。`/chat` 会补全已有联系人，`/pair` 会补全待处理设备。
...
3. 已有设备输入 `/pair`；存在多个请求时使用 `/pair <设备ID>`。
4. 两台设备比较界面显示的 SAS 短认证码。
5. 确认一致后，两边分别输入 `/confirm`。
6. 已有设备批准新设备，并通过认证通道迁移账号主密钥、联系人信任和可用历史。

不要只根据服务端或同一个聊天窗口传来的数字确认配对码，应使用独立可信渠道比较。
...
保持新设备在线，在已有设备上执行 `/pair [设备ID]`，比较短认证码后双方执行 `/confirm`。
